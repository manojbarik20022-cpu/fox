...
Используется браузерными провайдерами (Flow, Grok) — на каждый провайдер
заводится один или несколько аккаунтов, у каждого свой профиль Playwright и свой
счётчик израсходованных кредитов за день.

Nano Banana работает *внутри* Flow и не имеет отдельных аккаунтов —
использует те же Flow-профили, кредитов не списывает.
...
# Дневные лимиты по провайдерам (в «кредитах»). Стоимость каждой операции — в COST.
DAILY_LIMITS: dict[str, int] = {
    "flow": 50,   # 50 кредитов/день на бесплатном тарифе (хватает на 5 видео Veo или 50 картинок Imagen)
    "grok": 100,  # условно: считаем запросы для ротации (Grok без жёсткого «кредитного» лимита)
}

# Стоимость операций (кредитов). 0 = бесплатно.
#
# Flow по умолчанию печатает 2 выхода за Generate (Outputs per prompt = 2),
# поэтому "видео" стоит 2 × 5 = 10 кредитов. Это и есть «50 кредитов / 5 видео в день».
COST: dict[str, int] = {
    "flow:image": 1,        # Imagen в Flow — 1 кредит за картинку
    "flow:video": 10,       # Veo с дефолтными 2 выходами = 10 кредитов / Generate
    "flow:nano_banana": 0,  # Nano Banana внутри Flow не списывает кредиты
    "grok:image": 1,
    "grok:video": 5,
}
